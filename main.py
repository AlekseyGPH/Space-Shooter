...
for i in range(9):
    ...
    img_large = pygame.transform.scale(img, (72,72))
    explosion_anim["large"].append(img_large)
    img_small = pygame.transform.scale(img, (24,24))
...
def splash_screen(screen):
    ...
    splash.image = pygame.image.load("images/splash.jpg")
...
while run:
    ...
    #щита и метеоров
    shield_hits = pygame.sprite.spritecollide(shield, meteor_sprites, True)
    for meteor in hits:
        #спавн нового метеора взамен удаленного
        new_meteor = Meteor()
        all_sprites.add(new_meteor)
    ...
    for meteor in bullet_hits:
        ...
        # ДЕЛАЕМ взырыв на месте подбитого метеора
        if meteor.rect.width > 40:
            exp = Explosion(explosion_anim, meteor.rect.center, "large")
            all_sprites.add(exp)
        else:
            exp = Explosion(explosion_anim, meteor.rect.center, "small")
            all_sprites.add(exp)
